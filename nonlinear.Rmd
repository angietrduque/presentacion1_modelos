...
<center>
```{r warning=FALSE, include=F, paged.print=F}
G4 <- ggplot(Auto, aes(x = horsepower, y = mpg)) +
  geom_point(colour = "black") +
  stat_smooth(method = "lm", formula = y ~ poly(x, 1), colour = "blue", se = FALSE) +
  stat_smooth(method = "lm", formula = y ~ poly(x, 2), colour = "red", se = FALSE) +
  stat_smooth(method = "lm", formula = y ~ poly(x, 5), colour = "green", se = FALSE) +
  stat_smooth(method = "lm", formula = y ~ poly(x, 10), colour = "yellow", se = FALSE)
```
